...
namespace ICStars.Data
{
    ...
    public sealed class SqlHelper
    {
        ...
        /// <returns></returns>
        public static bool BulkCopy<T>(string connectionString, IEnumerable<T> list)
        {
            using (var connection =
                new SqlConnection(connectionString))
            {
                connection.Open();
                using (var sqlbulkTransaction = connection.BeginTransaction())
                {
                    using (var bulkCopy = new SqlBulkCopy(connection, SqlBulkCopyOptions.CheckConstraints, sqlbulkTransaction))
                    {
                        //bulkCopy.BatchSize = 500;
                        bulkCopy.DestinationTableName =
                            typeof (T).Name;
                        var dt = ReflectionHelper.BuildTableByModelCollection(list, true);
                        try
                        {
