<%@ Page Title="" Language="C#" MasterPageFile="~/SHTracker/_Layout.Master" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="ICStars2_0.SHTracker.CSSP.List" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="Main" runat="server">
 <h2>
        <span>Student Management</span></h2>
    <table class="toolbar">
        <tr>
        ...
        <tr>
            ...
            <td align="left" style="text-align:left;">
                Enter Text : <input type="text" id="txtSearch" minlength="2" class="required" />
    ...
    <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
        <tr align="center">
           ...
           <th>Last UpdateTime</th>
              <th  style="width:260px" align="center">
                Operation
            </th>
        </tr>
        ...
        <tr>
            ...
            <td align="center" valign="top">
             ...
             <td align="center" valign="top">
             <a href="#" class="button" onclick=" clearFrameCach('dialog<%#Eval("ID") %><%#Eval("CBOID") %><%#Eval("Quater_Year").ToString().Replace(" ",string.Empty) %>');$('#dialog<%#Eval("ID") %><%#Eval("CBOID") %><%#Eval("Quater_Year").ToString().Replace(" ",string.Empty) %>').dialog({width:450,height:520,close: function(event, ui) { window.location.reload(); }});return false;">Report Hours</a>
                 <a href="Search.aspx?StudentID=<%#Eval("ID") %>" class="button">Build Report</a>
             <a href="Edit.aspx?id=<%#Eval("ID") %>" class="button">Edit</a>
             <asp:LinkButton ID="LinkButton1" CssClass="button" CommandArgument='<%#Eval("ID") %>' OnClientClick='return TipOption(this,"Are you sure to delete this student [<em>"+$(this).parent().siblings().find("#span_username").html()+"</em>]?")' OnClick="lbtDeleteMember_Click" runat="server">Delete</asp:LinkButton><br/>
            ...
            <a href="#" class="button" onclick='importHistoricalHours(<%#Eval("ID") %>,<%#Eval("CBOID") %>,<%#Eval("CourseID") %>,"<%#Eval("FirstName")%> <%#Eval("LastName")%>")'>Import Historical Hours</a>
                ...
                <div class="for-dialog" id='dialog<%#Eval("ID") %><%#Eval("CBOID") %><%#Eval("Quater_Year").ToString().Replace(" ",string.Empty) %>' title="Report Hours For <%#Eval("FirstName")%> <%#Eval("LastName")%>">
                <iframe frameborder="0" scrolling="no" width="100%" height="100%" src="<%=Config.SHTrackerRoot %>/CSSP/ReportHours.aspx?sid=<%#Eval("ID") %>&cboid=<%#Eval("CBOID") %>&unlog=1&quarter=<%#Eval("Quater_Year") %>"></iframe>
                </div>
