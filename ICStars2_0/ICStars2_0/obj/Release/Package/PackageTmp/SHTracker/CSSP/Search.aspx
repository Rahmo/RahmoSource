<%@ Page Title="" Language="C#" MasterPageFile="~/SHTracker/_Layout.Master" AutoEventWireup="true" CodeBehind="Search.aspx.cs" Inherits="ICStars2_0.SHTracker.CSSP.Search" %>
      ...
      <%if(CurrentServiceHoursCollection != null)
        {
            ...
            {
      %>
        <h2 class="toolbar">CBO: <%=student2Cbo.CBOName %> <em><%=student2Cbo.Quater_Year %></em></h2>
                    Service Hours Total :
                <%=student2Cbo.ServedTotalHours %>/<%=student2Cbo.RequiredHours %><br/>
        <table id="hours_list" width="100%" border="0" align="center" cellpadding="0" cellspacing="0" style="float:left;">
        <tr><th>Quarter</th><th>Date</th><th>Hours Served</th><th>Type of Engagement</th><th>Description of Engagement</th> <th>Notes</th><th>Operation</th></tr>
<%
                foreach (ServiceHours serviceHours in CurrentServiceHoursCollection.Where(m => m.CBO_ID == student2Cbo.CBOID && m.CourseID == CurrentStudent.CourseID && m.Quater_Year.Equals(student2Cbo.Quater_Year)))
            {
                %>
                <tr>
                    ...
                    <td>
                        <%=serviceHours.HoursServed %>
                    </td>
                    <td>
                        <%=serviceHours.ContactType %>
                    </td>
                    <td><%=serviceHours.EngagementDescription %></td>
                    <td>
                        <%=serviceHours.Notes %>
                    </td>
                    <td>
                        <a href="#" onclick=" clearFrameCach('dialog<%=serviceHours.ID %>');$('#dialog<%=serviceHours.ID %>').dialog({width:450,height:470,close: function(event, ui) { window.location.reload(); }});return false;">Edit</a>
                        <div class="for-dialog" id="dialog<%=serviceHours.ID %>" title="Edit Hours">
                <iframe frameborder="0" scrolling="no" width="100%" height="100%" src="<%=Config.SHTrackerRoot %>/CSSP/ReportHours.aspx?id=<%=serviceHours.ID %>"></iframe>
                </div>
                    </td>
                </tr>
                <%
